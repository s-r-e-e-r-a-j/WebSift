...
install_dependencies() {
    ...
    if [ -d "/data/data/com.termux/files/usr" ]; then
        ...
        # For Linux environments (ubuntu,kalilinux,parrot os and other linux based distributions)
...
if [[ $target_url =~ $url_pattern ]]; then 
    ...
    if [[ "$email_option" =~ ^[Yy]$ || "$phone_option" =~ ^[Yy]$ ]]; then
...
rm temp_file.txt
if [[ -f "email_output.txt" || -f "phone_output.txt" ]]; then
    sleep 0.4
...
sleep 0.4
echo -e "${BrightWhite}[${BrightRed}!${BrightWhite}] ${BrightRed}Exiting....\n"
rm email_output.txt phone_output.txt 2> /dev/null 
exit
}
